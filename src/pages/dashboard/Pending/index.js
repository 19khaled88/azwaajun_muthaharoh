...
const Account = () => {
            ...
            <td className="px-1">
              ...
              <div class="relative inline-flex">
                ...
                <select onChange={(e) => handleStatus(e, item.id)} class={`rounded-full text-gray-600 h-8 pl-3 pr-5 text-sm  ${item.status === 'APPROVED' ? 'bg-green-100' : item.status === 'PENDING' ? 'bg-red-100' : 'bg-green-100'} hover:border-gray-400 focus:outline-none appearance-none`}>
                  <option className={``} value={item.status}>{item.status}</option>
                  {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="py-2 pl-2 text-sm text-gray-600">Admin panel / Pending </div>
      <div>
        <div class="w-full">

          <div class="px-2 py-4">
            <div class="flex items-center justify-between">
              <p tabindex="0" class="focus:outline-none text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-gray-800">Pending Bio Data({bio.length})</p>
